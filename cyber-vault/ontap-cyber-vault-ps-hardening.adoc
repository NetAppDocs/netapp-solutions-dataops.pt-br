...
O ONTAP fornece um conjunto de controles que permitem proteger o armazenamento ONTAP .  Use olink:https://docs.netapp.com/us-en/ontap/ontap-security-hardening/security-hardening-overview.html["orientações e configurações para ONTAP"^] para ajudar a organização a atingir os objetivos de segurança prescritos para confidencialidade, integridade e disponibilidade do sistema de informação.
...
. Crie um novo IPspace para isolar o tráfego de rede.
. Crie um novo SVM residindo no novo IPspace.
. Garanta que as políticas de roteamento do firewall estejam configuradas corretamente e que todas as regras sejam auditadas e atualizadas regularmente, conforme necessário.
...
. Proteja a administração com a Verificação Multiadministradora (MFA)
. Habilite a criptografia para dados padrão "em trânsito" entre clusters.
. SSH seguro com criptografia forte e imponha senhas seguras.
. Habilitar FIPS global.
. Telnet e Remote Shell (RSH) devem ser desabilitados.
. Bloquear conta de administrador padrão.
. Desabilite LIFs de dados e proteja pontos de acesso remoto.
. Desabilite e remova protocolos e serviços não utilizados ou estranhos.
. Criptografe o tráfego de rede.
. Use o princípio do menor privilégio ao configurar funções de superusuário e administrativas.
. Restrinja HTTPS e SSH de endereços IP específicos usando a opção de IP permitido.
. Desative e retome a replicação com base no cronograma de transferência.


Os marcadores 1 a 4 precisam de intervenção manual, como designar uma rede isolada, segregar o espaço IP e assim por diante, e precisam ser executados previamente.  Informações detalhadas para configurar o endurecimento podem ser encontradas emlink:https://docs.netapp.com/us-en/ontap/ontap-security-hardening/security-hardening-overview.html["Guia de reforço de segurança ONTAP"^] .  O restante pode ser facilmente automatizado para fins de implantação e monitoramento.  O objetivo dessa abordagem orquestrada é fornecer um mecanismo para automatizar as etapas de proteção para proteger o controlador do cofre no futuro.  O período de tempo em que o cofre cibernético fica aberto é o mais curto possível.  O SnapVault utiliza a tecnologia incremental para sempre, que moverá apenas as alterações desde a última atualização para o cofre cibernético, minimizando assim o tempo que o cofre cibernético deve permanecer aberto.  Para otimizar ainda mais o fluxo de trabalho, a abertura do cofre cibernético é coordenada com o cronograma de replicação para garantir a menor janela de conexão.
