...
summary: Orientação para ajudar a dimensionar um cofre cibernético ONTAP 
---
...
. Qual é a largura de banda e a latência entre a origem e o cofre cibernético?
. Qual é o tamanho dos arquivos e quantos arquivos são?
. Qual é sua meta de tempo de recuperação?
. Quantos dados você precisa recuperar dentro do RTO?
. Quantos relacionamentos de fãs do SnapMirror o cofre cibernético irá ingerir?
. Haverá recuperações únicas ou múltiplas acontecendo ao mesmo tempo?
. Essas múltiplas recuperações ocorrerão no mesmo primário?
. O SnapMirror será replicado para o cofre durante uma recuperação de um cofre?
...
image:ontap-cyber-vault-sizing.png["Opções de configuração do cofre cibernético"]
