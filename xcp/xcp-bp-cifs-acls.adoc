...
summary: Esta seção aborda o procedimento passo a passo para migrar dados CIFS com informações de segurança de um sistema ONTAP de origem para um sistema ONTAP de destino. 
---
= Migração de dados CIFS com ACLs de uma caixa de armazenamento de origem para ONTAP
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Esta seção aborda o procedimento passo a passo para migrar dados CIFS com informações de segurança de um sistema ONTAP de origem para um sistema ONTAP de destino.
...
NOTE: Se não houver nenhum agregado de dados, crie um novo usando o `storage aggr create` comando.
...
....
. Crie um LIF de dados para atender às solicitações de clientes de pequenas e médias empresas.
+
....
...
2 entries were displayed.
....
. Monte o volume de dados de destino no namespace SVM.
+
....
...
....
. Modifique as regras da política de exportação para permitir acesso aos clientes CIFs.
+
....
...
2 entries were displayed.
....
. Conecte-se ao sistema cliente Windows onde o XCP está instalado.  Navegue até o caminho de instalação do XCP.
+
....
C:\WRSHDNT>dir c:\netapp\xcp
dir c:\netapp\xcp
...
....
. Consulte as exportações SMB do nó de origem executando o comando `xcp show` comando no sistema host do cliente XCP Windows.
+
....
...
....
. Execute o `help` comando para cópia.
+
....
...
....
. No sistema ONTAP de destino, obtenha a lista de nomes de usuários e grupos locais que você precisa fornecer como valores para o `fallback-user` e `fallback-group` caminho de argumentos.
+
....
...
5 entries were displayed
....
. Para migrar os dados CIFs com ACLs da origem para o destino, execute o `xcp copy` comando com o `-acl` e `–fallback-user/group` opções.
+
Para o `fallback-user/group` opções, especifique qualquer usuário ou grupo que possa ser encontrado no Active Directory ou usuário/grupo local no sistema de destino.
...
C:\WRSHDNT>
....
. Se `xcp copy` resulta na mensagem de erro `ERROR failed to obtain fallback security principal` , adicione a caixa de destino no arquivo hosts(`C:\Windows\System32\drivers\etc\hosts` ).
+
Use o formato a seguir para a entrada da caixa de destino de armazenamento do NetApp .
...
....
. Se você ainda receber a mensagem de erro `ERROR failed to obtain fallback security principal` depois de adicionar a entrada da caixa de destino nos arquivos de hosts, o usuário/grupo não existe no sistema de destino.
+
....
...
C:\WRSHDNT>
....
. Usar `xcp copy` para migrar dados CIFs com ACLs (com ou sem a pasta raiz).
+
